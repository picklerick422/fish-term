...
// Probe whether an external CLI tool is available on this build host.
// Returns false when the binary is missing (spawn ENOENT) so the HNP layout
// patch can degrade gracefully on hosts without zip/unzip/zipinfo.
...
function patchBuiltHapHnpLayout(modulePath: string): void {
  ...
  // This relocation shells out to zipinfo/unzip/zip. If they are not installed
  // (e.g. a Windows host, or a HarmonyOS shell without the unzip package), skip
  // it with a clear warning instead of failing the build. The HNP stays bundled
  // at ets/hnp/; it just isn't moved to hnp/<abi>/, which only matters once the
  // local fish shell transport is wired up. Install `zip`/`unzip` to re-enable.
  const missing = ['zipinfo', 'unzip', 'zip'].filter((tool) => !hasTool(tool));
  if (missing.length) {
    console.warn(
      `WARN: Skipping HNP layout patch — missing build tool(s): ${missing.join(', ')}. ` +
      `The fish HNP remains bundled but is not relocated to hnp/arm64-v8a/. ` +
      `Install ${missing.join('/')} to enable it.`
    );
    return;
  }
